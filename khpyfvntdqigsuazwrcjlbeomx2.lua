...
local function VerificarOuCriarConfig(caminho)
    ...
    inicfg.save(ConfigPadrao, caminho)
end
...
local config = inicfg.load(ConfigPadrao, CaminhoConfig)
...
for secao, dados in pairs(ConfigPadrao) do
    if type(dados) == "table" then
        config[secao] = config[secao] or {}

        for chave, valor in pairs(dados) do
            if config[secao][chave] == nil then
                config[secao][chave] = valor
            end
        end
    else
        if config[secao] == nil then
            config[secao] = dados
        end
    end
end
...
imgui.OnInitialize(function()
    ...
    local function VerificarPeso(caminho, nome, minKB, maxKB)
        ...
        local tamanhoKB = file:seek("end") / 1024
        file:close()

        if tamanhoKB < minKB or tamanhoKB > maxKB then
            GUI.AbrirMenu[0] = false
            sampAddChatMessage(string.format("{FFFFFF}[{FF0000}HexDump{FFFFFF}] {FF0000}%s alterada!", nome), -1)
            return false
        end
        return true
    end

    if VerificarPeso(Foto1, "HexDumpTeam.png", 215, 217) then
        Imagem = imgui.CreateTextureFromFile(Foto1)
    end

    if VerificarPeso(Foto2, "JhowModsOfc.png", 35, 37) then
...
imgui.OnFrame(function() return GUI.AbrirMenu[0] end, function()
    ...
    if imgui.Begin("##menu", nil, windowFlags) then
        ...
        if GUI.temaAzul[0] then
            ...
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.00, 0.50, 1.00, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonActive,  imgui.ImVec4(0.00, 0.30, 0.60, 1.00))
        elseif GUI.temaRosa[0] then
            imgui.PushStyleColor(imgui.Col.Button,        imgui.ImVec4(1.00, 0.20, 0.70, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(1.00, 0.40, 0.80, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonActive,  imgui.ImVec4(0.80, 0.10, 0.50, 1.00))
        elseif GUI.temaVermelho[0] then
            imgui.PushStyleColor(imgui.Col.Button,        imgui.ImVec4(0.80, 0.00, 0.00, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(1.00, 0.00, 0.00, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonActive,  imgui.ImVec4(0.60, 0.00, 0.00, 1.00))
        end
        ...
        imgui.Dummy(imgui.ImVec2(0, 60 * DPI))
        ...
        if GUI.selected_category == "Jogador" then
            ...
            imgui.Dummy(imgui.ImVec2(0, 10 * DPI))
            if imgui.Checkbox(" ANT RVANK", GUI.AntRvank) then
                Som1()
                MostrarNotificacao("ANT RVANK", GUI.AntRvank[0])
            end
            imgui.Dummy(imgui.ImVec2(0, 25 * DPI))
            if GUI.temaAzul[0] then
                imgui.PushStyleColor(imgui.Col.Button,        imgui.ImVec4(0.00, 0.00, 1.00, 1.00))
            ...
            imgui.Dummy(imgui.ImVec2(0, 25 * DPI))
            if Toggle(" ATIVAR TELA ESTICADA", GUI.TelaEsticada) then
                Som1()
                MostrarNotificacao("TELA ESTICADA", GUI.TelaEsticada[0])
            end
            imgui.Dummy(imgui.ImVec2(0, 5 * DPI))
            if GUI.TelaEsticada[0] then
        ...
        for i, p in ipairs(LimparParticulas) do
            ...
            local corParticula
            if GUI.temaAzul[0] then
                corParticula = imgui.ImVec4(0.0, 0.6, 1.0, 1.0)
            elseif GUI.temaRosa[0] then
                corParticula = imgui.ImVec4(1.0, 0.2, 0.7, 1.0)
            else
                corParticula = imgui.ImVec4(1.0, 0.0, 0.0, 1.0)
            end
            local pointColor = imgui.GetColorU32Vec4(corParticula)
...
function main()
    ...
    sampRegisterChatCommand("hexdump", function()
        ...
        if VerSegundoArquivo() then
            GUI.AbrirMenu[0] = not GUI.AbrirMenu[0]
            GUI.selected_category = "creditos"
        else
            EnviarSmS("{FF0000}Voce precisa ter o arquivo HexDump Team Mobile Log.lua!!", -1)
        end
    end)
    while true do
        wait(0)
        ...
        if GUI.AtivarDraFov[0] and GUI.AtivarAimbot[0] and isPlayerArmed() and not IgnoreDrawFovArma() then
...
function IgnoreDrawFovArma()
    local weapon = getCurrentCharWeapon(PLAYER_PED)
    return weapon == 34
end

function se.onSetPlayerHealth()
    if GUI.AntHs[0] then
        freezeCharPosition(PLAYER_PED, true)
        freezeCharPosition(PLAYER_PED, false)
        setPlayerControl(PLAYER_HANDLE, true)
...
function isTargetAfkAim(playerId)
...
function isPlayerInVehicleAim(charProximo)
...
function isClearObject(myPos, enPos)
    if GUI.IgnoreObject[0] then
...
function EnviarSmS(text)
    tag = '{FF0000}[JhowModsOfc]: '
    sampAddChatMessage(tag .. text, -1)
end

function MostrarNotificacao(tipo, estado)
